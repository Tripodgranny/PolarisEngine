...
void setupColours(Material material, vec4 total) {
    if (hasTexture >= 1) {
        ambientC = total;
        diffuseC = ambientC;
        specularC = ambientC;
...
void main() {
    ...
    vec2 tiledCoords = fragTextureCoord * 46.0;
    ...
    setupColours(material, totalColour);
    ...
    fragColour = ambientC * vec4(ambientLight, 1) * totalColour + diffuseSpecularComp;
