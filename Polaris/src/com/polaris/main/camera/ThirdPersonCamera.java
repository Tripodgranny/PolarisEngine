...
public class ThirdPersonCamera extends Camera {
	...
	private final float DEFAULT_PITCH = 35f;
	private final float MINIMUM_PITCH = 0f;
	private final float MAXIMUM_PITCH = 90f;

	private Entity entity;
	
	private float distanceFromPlayer = 125f;
	...
	private void calculateCameraPosition(float horizD, float vertD) {
		float theta = entity.transform.rotation.y + angleAroundPlayer;
		float offsetX = (float) (horizD * Math.sin(Math.toRadians(theta)));
		float offsetZ = (float) (horizD * Math.cos(Math.toRadians(theta)));
		setPosition(entity.transform.position.x - offsetX,
					entity.transform.position.y + vertD,
					entity.transform.position.z - offsetZ);
	}
	...
	private void calculateYaw() {
		this.yaw = 180 - (entity.transform.rotation.y + angleAroundPlayer);
	}
